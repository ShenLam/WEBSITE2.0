...
test(`TC_VRF01: Verify that the user can successfully make a payment using “THẺ ATM & TÀI KHOẢN NGÂN HÀNG”`, async ({ page }) => {
...
test(`TC_VRF02: Verify that the user can successfully cancel the payment using “THẺ ATM & TÀI KHOẢN NGÂN HÀNG”`, async ({ page }) => {
    const waitFunction = new WaitFunction(page);
...
test(`TC_VRF03: Verify that the user can successfully cancel the payment using “THANH TOÁN BẰNG MÃ QR”`, async ({ page }) => {
    const waitFunction = new WaitFunction(page);
    ...
    await test.step('Verify payment error', async () => {
        ...
        await expect(page.locator('div.box-error-inner').locator('img')).toBeVisible();
        await expect(page.locator('div.box-error-inner').getByText('Thông báo')).toBeVisible();
        await expect(page.locator('div.box-error-inner').getByText('Ngân hàng thanh toán không được hỗ trợ')).toBeVisible();
    })
});

test(`TC_VRF04: Verify that the user can successfully cancel the payment using “VÍ ĐIỆN TỬ”`, async ({ page }) => {
    const waitFunction = new WaitFunction(page);
